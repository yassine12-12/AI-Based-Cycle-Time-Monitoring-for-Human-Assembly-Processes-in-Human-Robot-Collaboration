...
def get_step_configuration(step_name: str) -> Optional[Dict[str, Any]]:
    ...
    step_configs = {
        ...
        'Human mounts teil B on A': {
            'completion_type': 'aruco_mit_kiste',
            'distance_threshold': 150,
            'wait_duration': 2,
            'left_plate_id': 584,
            'requires_hands_away': True,
            'required_objects': ['Sensorbaugruppe'] # last step
        },
        'Human mounts rad': {
...
class StepClock:
    ...
    def __init__(self, fps: float):
        self.step_start_frames = {}
        self.step_durations = {}
        self.current_step_start_frame = None
        self.fps = fps

    def start_step(self, step_name: str, current_frame_idx: int):
        self.current_step_start_frame = current_frame_idx # self.current_step_start = time.time()
        self.step_start_frames[step_name] = current_frame_idx # self.step_start_times[step_name] = self.current_step_start

    def end_step(self, step_name: str, current_frame_idx: int):
        if step_name in self.step_start_frames and self.current_step_start_frame is not None:
            duration_frames = current_frame_idx - self.current_step_start_frame 
            duration = duration_frames / self.fps # duration = time.time() - self.current_step_start
            self.step_durations[step_name] = duration
            self.current_step_start_frame = None

    def get_current_step_duration(self, current_frame_idx: int) -> float:
        if self.current_step_start_frame is not None:
            duration_frames = current_frame_idx - self.current_step_start_frame
            return duration_frames / self.fps # return time.time() - self.current_step_start
        return 0.0
...
def generic_step_logic(step_name: str, hand_landmarks, yolo_results, 
    ...
    if completion_type == 'distance_timeout':
        ...
        if hands_near and not step_manager.begin and not is_kiste_empty:
        ...
        # detected_ids, _, _ = detect_aruco_ids(current_frame,dict_types=None)# [aruco.DICT_4X4_1000]
        left_id = config.get('left_plate_id', 584)
        print(detected_ids)
        if not step_manager.begin:
            if left_id not in detected_ids:
                print("1111111111111111")
                if step_manager.missing_start_time is None:
                    print("222222222222222")
                    step_manager.missing_start_time = time.time()  # Start missing timer
                elif (time.time() - step_manager.missing_start_time) > 1.0 and not is_kiste_empty:
                    print("3333333333333333")
                    step_manager.begin = True
                    step_manager.missing_start_time = None
                print("4444444444444")
            else:
                print("555555555555555")
                step_manager.missing_start_time = None  # Reset if ID is detected again

        if requires_hands_away and is_kiste_empty:
            if step_manager.begin and hands_near and not step_manager.end:
                step_manager.end = True 
            
            print("step_manager.end", step_manager.end)
...
def human_step_logic(current_step: AssemblyStep, hand_landmarks, yolo_results, 
                    ...
                    detection_smoother: DetectionSmoother=None, current_frame=None, detected_ids=None) -> bool:
    """
    ...
    if config:
        ...
        return generic_step_logic(current_step.name, hand_landmarks, yolo_results, 
                                ...
                                current_frame=current_frame, detected_ids=detected_ids)
